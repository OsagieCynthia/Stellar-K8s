...
"#,
    ...
    wait_for("PDB created", Duration::from_secs(90), || {
        Ok(run_cmd(
            "kubectl",
            &["get", "pdb", validator_name, "-n", TEST_NAMESPACE],
        )
        .is_ok())
    })?;
    ...
    if min_available != "2" {
        return Err(format!(
            "Expected minAvailable to be 2 for 3 replicas, got {}",
            min_available
        )
        .into());
    }
...
"#
    ...
    wait_for(
        "Running after disk-full recovery",
        Duration::from_secs(120),
        || {
            let phase = run_cmd(
                "kubectl",
